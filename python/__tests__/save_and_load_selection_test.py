...
class FakeJsonable(Jsonable):
    ...
    def __eq__(self, other: Any) -> bool:
        if (isinstance(other, FakeJsonable)):
...
class TestSaveAndLoadSettings(fake_filesystem_unittest.TestCase):
    ...
    def setUp(self):
        ...
        JSONABLE_1A = FakeJsonable("name_1a", 10)
        JSONABLE_1B = FakeJsonable("name_1b", 11)

        JSONABLE_2A = FakeJsonable("name_2a", 9493)
        JSONABLE_2B = FakeJsonable("name_2b", 9494)
    ...
    def test_loading_from_directory_that_does_not_exist(self):
        with self.assertRaises(FileNotFoundError):
            load(self.non_existent_directory, FakeJsonable)

    def test_loading_from_a_file(self):
        with self.assertRaises(NotADirectoryError):
            load(self.existent_file, FakeJsonable)

    def test_loading_from_directory_with_no_save_data(self):
        EMPTY_SELECTION = Selection()
        SELECTION = load(self.existent_directory, FakeJsonable)
    ...
    def test_overwrite_previous_save_with_non_empty_selection(self):
        save(self.selection_1, self.existent_directory)
    ...
    def test_overwrite_previous_save_with_empty_selection(self):
        EMPTY_SELECTION = Selection()

        save(self.selection_1, self.existent_directory)
        save(EMPTY_SELECTION, self.existent_directory)

        SELECTION = load(self.existent_directory, FakeJsonable)

        self.assertEqual(SELECTION, EMPTY_SELECTION)
