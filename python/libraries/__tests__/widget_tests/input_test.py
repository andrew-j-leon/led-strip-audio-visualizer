...
from libraries.widget import Input, Widget
...
class InputTestCase(unittest.TestCase):
    ...
    def setUp(self):
        self.input_with_key = Input(self.KEY, self.TEXT, self.ENABLED, self.WIDTH)
        self.input_with_no_key = Input(self.NO_KEY, self.TEXT, self.ENABLED, self.WIDTH)
...
class TestConstructor(InputTestCase):
    def test_with_key(self):
        ...
        self.assertEqual(self.input_with_key.width, self.WIDTH)
    ...
    def test_with_no_key(self):
        ...
        self.assertEqual(self.input_with_key.enabled, self.ENABLED)
        self.assertEqual(self.input_with_no_key.width, self.WIDTH)
...
class TestSetters(InputTestCase):
    ...
    def test_setting_width(self):
        self.input_with_key.width = self.NEW_WIDTH

        self.assertEqual(self.input_with_key.width, self.NEW_WIDTH)


class TestRepr(InputTestCase):
    def test_with_key(self):
        EXPECTED = f'Input(key={self.KEY}, text={self.TEXT}, enabled={self.ENABLED}, width={self.WIDTH})'

        self.assertEqual(EXPECTED, repr(self.input_with_key))

    def test_with_no_key(self):
        EXPECTED = f'Input(key={self.NO_KEY}, text={self.TEXT}, enabled={self.ENABLED}, width={self.WIDTH})'
...
class TestEqual(InputTestCase):
    def test_with_key(self):
        EQUAL_INPUT = Input(self.KEY, self.TEXT, self.ENABLED, self.WIDTH)
        ...
        UNEQUAL_PARAMETERS = [{'key': self.NEW_KEY, 'text': self.TEXT, 'enabled': self.ENABLED, 'width': self.WIDTH},
                              {'key': self.KEY, 'text': self.NEW_TEXT, 'enabled': self.ENABLED, 'width': self.WIDTH},
                              {'key': self.KEY, 'text': self.TEXT, 'enabled': self.NEW_ENABLED, 'width': self.WIDTH},
                              {'key': self.KEY, 'text': self.TEXT, 'enabled': self.ENABLED, 'width': self.NEW_WIDTH}, ]

        self.check_not_equal_with_and_without_key(self.input_with_key, UNEQUAL_PARAMETERS)

    def test_with_no_key(self):
        EQUAL_INPUT = Input(self.NO_KEY, self.TEXT, self.ENABLED, self.WIDTH)
        ...
        UNEQUAL_PARAMETERS = [{'key': self.NO_KEY, 'text': self.NEW_TEXT, 'enabled': self.ENABLED, 'width': self.WIDTH},
                              {'key': self.NO_KEY, 'text': self.TEXT, 'enabled': self.NEW_ENABLED, 'width': self.WIDTH},
                              {'key': self.NO_KEY, 'text': self.TEXT, 'enabled': self.ENABLED, 'width': self.NEW_WIDTH}]
